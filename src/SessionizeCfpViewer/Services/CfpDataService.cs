...
public class CfpDataService
{
    ...
    public Task<List<CfpSessionFlat>> GetAllSessionsAsync()
    {
        return Task.FromResult(_sessions.ToList());
    }

    public Task<CfpSessionFlat?> GetSessionByIdAsync(string id)
    {
        if (int.TryParse(id, out var intId))
        {
            var session = _sessions.FirstOrDefault(s => s.Id == intId);
            return Task.FromResult(session);
        }
        return Task.FromResult<CfpSessionFlat?>(null);
    }

    public Task<List<CfpSessionFlat>> SearchSessionsAsync(
        ...
        bool ascending = true)
    {
        ...
        if (!string.IsNullOrWhiteSpace(searchTerm))
        {
            ...
            query = query.Where(s =>
                ...
                (!string.IsNullOrEmpty(s.LocationFull) && s.LocationFull.ToLower().Contains(searchTerm)) ||
        ...
        query = sortBy?.ToLower() switch
        {
            ...
            "country" => ascending ? query.OrderBy(s => s.LocationCountry) : query.OrderByDescending(s => s.Country),
